...
test_that("reads an html table", {
  filename <- system.file(package = "contactparser", "inst", "testdata", "example.html")
  dat <- .read_html_table(filename)
  expect_is(dat, "tbl_df")
})

test_that("reads a csv table", {
  filename <- system.file(package = "contactparser", "inst", "testdata", "example.csv")
  expect_silent(dat <- .read_csv_table(filename))
  expect_is(dat, "tbl_df")
})

test_that("produces same output for csv and html", {
  html_filename <- system.file(package = "contactparser", "inst", "testdata", "example.html")
  html <- .read_html_table(html_filename)

  csv_filename <- system.file(package = "contactparser", "inst", "testdata", "example.csv")
